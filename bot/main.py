...
def handle_message(update: Update, _: CallbackContext):
    ...
    it_keywords = r'\b(ошибк[а-я]*|сбо[йи]*|неполадк[а-я]*|не\s*работа[ею][тт]*|загрузк[а-я]*|установк[а-я]*|активаци[я-я]*|логин[а-я]*|парол[ьяи]*|вирус[а-я]*|безопасност[ьи]*|сетев[ооеё]*\s*подключен[иеия]*|браузер[а-я]*|(электронн[а-я]*\s*)?почт[а-я]*|синхронизац[ия-я]*|резервн[а-я]*\s*копирован[иеия]*|восстановлен[иеия-я]*\s*данных|настройк[а-я]*)\b'
